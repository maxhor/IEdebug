...
var qtService = (function () {
    ...
    qtService.prototype.set_iconcolors = function () {
        ...
        var _colors = this.rowsvalues.forEach(function (rowsvalue) {
            artnr = rowsvalue.artikelnr;
            index = _this.formdef["bootstrapcolumns"][_this.btc_idx].rows.findIndex(function (row) {
                artnr2 = row["values"][_this.art_idx];
                if (artnr == artnr2 && row["values"][_this.not_counted_idx] == "1")
                    return true;
            }, _this);
            if (index >= 0)
